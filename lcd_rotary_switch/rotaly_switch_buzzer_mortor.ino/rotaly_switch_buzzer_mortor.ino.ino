...
int ledPin_2 = 3;       // LEDのコネクタ番号（D3につないだ場合）
int ledPin_3 = 8;       // LEDのコネクタ番号（D8につないだ場合）
int buzzerPin = 6;        // ブザーのコネクタ番号（D6につないだ場合）
const int rotaryPin = A0;                    // rotary angle sensor connect to A0
int motorPin = 7;        // モータのコネクタ番号（D8につないだ場合）
...
void setup() {
  Serial.begin(9600);                         // set the serial communication frequency at 9600 bits per sec
    ...
    pinMode(ledPin_3, OUTPUT);  // led3をOUTPUTとして設定
    
    pinMode(rotaryPin, INPUT);
    pinMode(buttonPin, INPUT);  // ポタンをINPUTとして設定

    Motor.attach(motorPin);     // サーボモータの接続
    Motor.write(120);         // サーボモータを120度の位置に設定（0度だと微振動するかもしれないので…）
...
void judge_switch(int input_second){
  if (pushed_count == 0 && input_second != 0){  //入力秒数が0の時にスイッチ押しても何も起きないようにする
...
//    digitalWrite(ledPin_2, HIGH);
    ...
    if (buttonState == 0){
        degrees = getDegree();
        seconds =30 - (degrees / 10);   //秒数の入力を時計回りにする
        if (seconds <= 9){
            Serial.println("1keta");
            lcd.print(clear_str);   
            lcd.setCursor(7, 0);
            lcd.print(seconds);
        }else{
    ...
    judge_switch(seconds);



//これやと、0secondsでボタン押してまうとロータリーエンコーダで入力秒数を変更できなくなってしまう
    if (buttonState == 1){
        ...
        if (seconds != 0 && flag_end == 0){
          tone(buzzerPin, 200*40);  //523*32
